...
| 0.28    | 14/05/2026  | **Sistema de literales dinámicos implementado** (ML63.html v11). Todos los textos visibles (KPIs, desgloses de gastos, auditoría, resúmenes tesorería) ahora se generan dinámicamente mediante función helper `setInner(id, html)` que interpola datos reales de Supabase. Sustituye approach antiguo de hardcodear valores estáticos en HTML. 36 puntos de inyección identificados en `loadDashboardData()` (líneas 1615-1752): tesorería 2026/2025, gastos operativos/mejoras, nóminas portero, despido/embargos, energía (luz/agua/calefacción), limpieza, auditoría footer, y todos los grids de KPIs por año/categoría. Implementación por Gemini, estructura simple sin framework i18n: genera contenido en el momento de cargar datos, redibuja UI automáticamente. |

> Cualquier modificación posterior se anota aquí con resumen del cambio.
| 0.29    | 13/06/2026  | **Refactorización del Motor de Auto-Conciliación**. El motor de cruce automático (`processBancoFile` en `ML63.html`) ha sido reescrito para ejecutarse de forma **Global e Incondicional**. Ya no requiere que el archivo Excel importado tenga registros nuevos para ejecutarse. Ahora evalúa constantemente todos los proyectos pendientes contra todos los movimientos huérfanos cada vez que se hace drag & drop. Incluye heurísticas de márgenes de IVA (10% y 21%) sobre presupuestos netos. Se prohíbe explícitamente a futuros agentes simplificar este código. |
